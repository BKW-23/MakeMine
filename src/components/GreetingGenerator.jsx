...
export default function GreetingGenerator({ productName, onConfirm }) {
  ...
  const generate = async (e) => {
    ...
    setConfirmed(false);
    try {
      const res = await api.functions.invoke("generateGreeting", { ...form, productName });
      ...
      {!confirmed && (
        <form onSubmit={generate} className="grid gap-3">
          <div className="grid grid-cols-2 gap-3">
            <input value={form.recipient} onChange={(e) => setForm({ ...form, recipient: e.target.value })} placeholder="Người nhận" className="rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-primary min-h-11" />
            <input value={form.relationship} onChange={(e) => setForm({ ...form, relationship: e.target.value })} placeholder="Mối quan hệ (bạn thân, người yêu...)" className="rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-primary min-h-11" />
          </div>
          <input value={form.occasion} onChange={(e) => setForm({ ...form, occasion: e.target.value })} placeholder="Dịp tặng (sinh nhật, kỷ niệm...)" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-primary min-h-11" />
          <input value={form.hobbies} onChange={(e) => setForm({ ...form, hobbies: e.target.value })} placeholder="Sở thích người nhận" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-primary min-h-11" />
          <input value={form.keywords} onChange={(e) => setForm({ ...form, keywords: e.target.value })} placeholder="Từ khoá tuỳ chọn (ngọt ngào, hài hước...)" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:border-primary min-h-11" />
